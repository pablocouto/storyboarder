  ...
  describe('can make a copy of a project from its referenced files', () => {
    before(function () {
      ...
      mockFs({
        [fixturesPath]: {
          'projects': {
            'multi-scene.fdx': fs.readFileSync(path.resolve(path.join(fixturesPath, 'projects', 'multi-scene.fdx'))),
    ...
    it('can copy a multi-scene project', () => {
      let srcFilePath = path.resolve(path.join(fixturesPath, 'projects', 'multi-scene.fdx'))
